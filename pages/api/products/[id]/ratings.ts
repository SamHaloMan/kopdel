...
const ratingListHandler = async (
    ...
    if (req.method === 'GET') {
        try {
            res.status(200).json(await getProductRatings(req));
        } catch (err:any) {
            console.error(err)
            res.status(500).json({
                message: err.message
        ...
        try {
            await removeProductRating(req, res);
        } catch (err:any) {
            console.error(err)
            res.status(500).json({
                message: err.message
...
async function getProductRatings(req: NextApiRequest) {
    // Get productID;
...
async function addProductRating(req: NextApiRequest, res: NextApiResponse<any>) {
    // Get productID;
    if (typeof req.query.id !== 'string' && typeof req.query.id !== 'number') {
        throw new Error('Invalid parameter `id`.');
    }
    ...
    try {
        ...
        if (err instanceof Prisma.PrismaClientKnownRequestError) {
            if (err.code === 'P2002') {
                res.status(200).json({
                    message: `User <${user.id}> has already rate for product <${productId}>.`
                });
            }
        } else {
            throw err
...
async function removeProductRating(req: NextApiRequest, res: NextApiResponse<any>) {
    // Get productID;
    if (typeof req.query.id !== 'string' && typeof req.query.id !== 'number') {
        throw new Error('Invalid parameter `id`.');
    }
