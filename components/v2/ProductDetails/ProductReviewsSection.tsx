...
import { productRatingQuery } from 'selectors';
import { ProductRatingsProps, starLabels } from 'const';
import { roundHalf } from 'lib/utils';
import HalfRating from 'components/v2/Rating/HalfRating';
import ProductRatingDeleteDialog from 'components/v2/ProductDetails/ProductRatingDeleteDialog';
import ProductAddRatingDialog from 'components/v2/ProductDetails/ProductAddRatingDialog';

export default function ProductReviewsSection() {
  const addRatingDialogRef = React.useRef<HTMLDialogElement>(null);

  const productRatingLoadable = useRecoilValueLoadable(productRatingQuery);
  const [productDetailsId] = useRecoilState(productDetailsIdState);

  switch (productRatingLoadable.state) {
    case 'hasValue':
      const data = productRatingLoadable.contents.content;
      return (
        <>
            ...
            <div className='hero-content items-start'>
              ...
              <div className='overflow-x-auto mt-16'>
                {data?.content?.length > 0 && (
                  <ReviewsTable content={data.content} productId={productDetailsId} />
                )}
              </div>
            </div>
          </div>
          <ProductAddRatingDialog
    ...
    case 'hasError':
      // throw productRatingLodable.contents;
      return <></>;
  }
}

const ReviewOverview = (props: { content: ProductRatingsProps[] }) => {
...
const ReviewsTable = (props: {
  content: ProductRatingsProps[];
  productId: string;
}) => {
  const { content, productId } = props;
